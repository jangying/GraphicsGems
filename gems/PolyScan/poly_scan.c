...
#define HUGE FLT_MAX
    ...
    {   /* scan in y, activating new edges on left & right */
        ...
        { /* do scanlines till end of l or r edge */
            ...
            {
                if (l.sx <= r.sx)
                    scanline(y, &l, &r, win, pixelproc, mask);
                else
                    scanline(y, &r, &l, win, pixelproc, mask);
            }
            y++;
...
static void scanline(y, l, r, win, pixelproc, mask) int y;
unsigned long mask;
Poly_vert *l, *r;
Window *win;
void (*pixelproc)();
...
static void incrementalize_y(p1, p2, p, dp, y, mask) register double *p1, *p2, *p, *dp;
register unsigned long mask;
int y;
{
    double dy, frac;
...
static void incrementalize_x(p1, p2, p, dp, x, mask) register double *p1, *p2, *p, *dp;
register unsigned long mask;
int x;
{
    double dx, frac;
...
static void increment(p, dp, mask) register double *p, *dp;
register unsigned long mask;
{
